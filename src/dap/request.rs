use super::{Command, StandardCommand};
...
impl<'a> Request<'a> {
    ...
    pub fn from_report(report: &'a [u8]) -> Option<Self> {
        ...
        let command = (*command)
            .try_into()
            .unwrap_or(Command::Standard(StandardCommand::Unimplemented));
    ...
    fn command_len(&self) -> Option<usize> {
        match self.command {
            Command::Standard(cmd) => match cmd {
                StandardCommand::DAP_Info => Some(1),
                StandardCommand::DAP_HostStatus => Some(2),
                StandardCommand::DAP_Connect => Some(1),
                StandardCommand::DAP_Disconnect => Some(0),
                StandardCommand::DAP_WriteABORT => Some(5),
                StandardCommand::DAP_Delay => Some(2),
                StandardCommand::DAP_ResetTarget => Some(0),
                StandardCommand::DAP_SWJ_Pins => Some(6),
                StandardCommand::DAP_SWJ_Clock => Some(4),
                StandardCommand::DAP_SWJ_Sequence => {
                ...
                StandardCommand::DAP_SWD_Configure => Some(1),
                StandardCommand::DAP_SWD_Sequence => {
                ...
                StandardCommand::DAP_SWO_Transport => Some(1),
                StandardCommand::DAP_SWO_Mode => Some(1),
                StandardCommand::DAP_SWO_Baudrate => Some(4),
                StandardCommand::DAP_SWO_Control => Some(1),
                StandardCommand::DAP_SWO_Status => Some(0),
                StandardCommand::DAP_SWO_ExtendedStatus => Some(1),
                StandardCommand::DAP_SWO_Data => Some(2),
                StandardCommand::DAP_JTAG_Sequence => {
                    let sequences = *(self.data.get(0)?);
                    let mut data = self.data.get(1..)?;
                    let mut total_len = 1;
                    for _ in 0..sequences {
                ...
                StandardCommand::DAP_JTAG_Configure => Some((*(self.data.get(0)?) + 1).into()),
                StandardCommand::DAP_JTAG_IDCODE => Some(1),
                StandardCommand::DAP_TransferConfigure => Some(5),
                StandardCommand::DAP_Transfer => {
                ...
                StandardCommand::DAP_TransferBlock => {
                    let transfers =
                ...
                StandardCommand::DAP_TransferAbort => Some(0),
                StandardCommand::DAP_ExecuteCommands => Some(self.data.len()),
                StandardCommand::DAP_QueueCommands => Some(self.data.len()),
                StandardCommand::Unimplemented => Some(self.data.len()),
            },
            Command::Vendor(cmd) => Some(4),
        }
    }
    ...
    pub fn next_sub_request(&mut self) -> Option<Self> {
        if let Command::Standard(StandardCommand::DAP_ExecuteCommands) = self.command {
